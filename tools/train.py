...
def train():
    with tf.Graph().as_default():
        ...
        temp3 = tf.maximum(tf.cast(tf.constant(0),tf.float64),tf.subtract(tf.cast(tf.constant(1),tf.float64),lr_x))
        lr = tf.add(tf.cast(min_lr,tf.float32),tf.multiply(tf.subtract(max_lr,min_lr),tf.cast(temp3,tf.float32)))
        ...
        with tf.Session(config=config) as sess:
            ...
            mkdir(summary_path)
            summary_writer = tf.summary.FileWriter('/content/drive/MyDrive/RDFPN_ckpt7/summary', graph=sess.graph)
            ...
            for step in range(cfgs.MAX_ITERATION):
                ...
                if (step > 0 and step % 5000 == 0) or (step == cfgs.MAX_ITERATION - 1):
                    ...
                    save_ckpt = os.path.join('/content/drive/MyDrive/RDFPN_ckpt7', 'voc_'+str(_global_step)+'model.ckpt')
